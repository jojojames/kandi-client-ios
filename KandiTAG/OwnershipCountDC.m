...
@implementation OwnershipCountDC
...
-(void)checkOwnershipCount {
    ...
    NSLog(@"checkOwnershipCount has been called");
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://www.kanditag.com/ownership_count.php?"]];
...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    NSLog(@"ownershipcount strings array: %@", strings);
    
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    
    NSLog(@"strings array count: %d", strings.count);
    
    if (strings.count > 6) {
        
        NSLog(@"maximum qr count reached");
        
        [defaults setBool:YES forKey:@"MAXQR"];
        
    }
    
    else if (strings.count < 6) {
        
        NSLog(@"qr can still be traded");
        
        [defaults setBool:NO forKey:@"MAXQR"];
